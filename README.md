...
- 박소영: 단순한 페이지 제작을 넘어 기획부터 배포까지의 전체 사이클을 경험하며 시야를 넓힐 수 있었다. 특히 팀원들과 Git으로 호흡을 맞추며 겪은 시행착오들은 혼자 공부할 때는 알 수 없었던 값진 경험이었다.
  팀원 간의 적극적인 소통 덕분에 프로젝트를 끝까지 성공적으로 완주할 수 있었지만, 기능 구현에 집중하다 보니 정작 코드를 다듬고 검증할 시간이 부족했기에 아쉬움이 남는다. '돌아가는 코드'를 넘어 '지속 가능한 코드'를 만들기 위한 리팩토링의 중요성을 다시 한번 절감한 프로젝트였다.
